...
void main() {
  ...
  test('serves index.html at the root', () async {
    final response = await http.get(Uri.parse('${h.base}/'));
  ...
  test('serves nested assets and 404s unknown paths', () async {
    expect(
      (await http.get(Uri.parse('${h.base}/main.dart.js'))).statusCode,
      200,
    );
    expect((await http.get(Uri.parse('${h.base}/nope.js'))).statusCode, 404);
  });
  ...
  test('refuses to serve outside the web root', () async {
    ...
    File(p.join(h.root.path, 'secret')).writeAsStringSync('do not serve me');
    ...
    final socket = await Socket.connect('localhost', h.server.port);
    ...
    // outside the web h.root is never served.
  ...
  test('a failing write reports a server error rather than crashing', () async {
    ...
    final blocked = Directory(p.join(h.root.path, 'blocked'))..createSync();
  ...
  test('labels wasm and js correctly', () async {
    ...
    final wasm = await http.get(Uri.parse('${h.base}/canvaskit.wasm'));
    expect(wasm.headers['content-type'], contains('application/wasm'));
    final js = await http.get(Uri.parse('${h.base}/main.dart.js'));
